...
void loopMKTSDisplay() {
    if (targetTime < millis()) {
        Serial.println("Looping display");
        ...
        tft.drawCentreString("12.34 is in font 6", 120, 92, 2);  // Draw text centre at position 120, 92 using font 2
...
void displayPH(const float pH, const float convertedPH, const float rawPH_mvag, const float calibratedPH_mvag, const ulong asOf) {
    ...
    tft.println(pH, 3);
    tft.print(F("mavg(pH)="));
    tft.println(rawPH_mvag, 3);

    tft.print(F("calib="));
    tft.println(convertedPH, 3);
    tft.print(F("mvag(calib)="));
    tft.println(calibratedPH_mvag, 3);
